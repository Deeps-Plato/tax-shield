...
async def start_questionnaire(db: AsyncSession, user: User, tax_year: int) -> dict:
    session_id = str(uuid.uuid4())

    # Get user's existing items to skip already-claimed areas
    existing = await db.execute(
        select(UserItem.item_id).where(UserItem.user_id == user.id, UserItem.tax_year == tax_year)
    )
    _ = {row[0] for row in existing.all()}  # reserved for future skip logic
...
async def answer_question(
    db: AsyncSession,
    user: User,
    answer: "QuestionnaireAnswer",  # noqa: F821
) -> dict:
    ...
    if current and current.get("discovered_category"):
        ...
        if cat and answer.response.lower().startswith("yes"):
            ...
            discovered_items = [
                {"id": i.id, "name": i.name, "description": i.description} for i in found_items
            ]
...
async def _ai_follow_up(
    db: AsyncSession,
    user: User,
    last_answer: "QuestionnaireAnswer",  # noqa: F821
) -> dict:
    ...
    prompt = f"""Based on this taxpayer's questionnaire responses, \
generate ONE more targeted question to discover additional tax \
deductions or credits they may be missing.

Previous responses:
{history}

Return JSON with keys: question (string), options (list of 3-4 \
strings), question_key (short snake_case identifier).
If no more questions are useful, return \
{{"is_final": true, "summary": "brief summary of discoveries"}}.
